...
{
	//원본 소스: http://www.devpia.com/Maeul/Contents/Detail.aspx?BoardID=217&MAEULNO=8&no=20472&page=10
	//자바 소스: http://blog.naver.com/PostView.nhn?blogId=qorduf&logNo=30096790093
	//CBC, ECB 설명: http://www.adayinthelifeof.nl/2010/12/08/encryption-operating-modes-ecb-vs-cbc/
	public class CSeed
	{
		#region SSN
		...
		{
			//nOutSize가 nInSize보다 작아져서 주석
			//int nOutSize = (int)((Data.Length) / 16) * 16;
			int nOutSize = ((Data.Length / 16) + ((Data.Length % 16) > 0 ? 1 : 0)) * 16;
			if (cbcPad)
			{
		...
		{
			return CSeed.Encrypt(Data, seedKey, false);
		}
		public static string Encrypt(string Decrypted, byte[] seedKey, bool cbcPad, Encoding Enc)
		{
			byte[] aDecrypted = Enc.GetBytes(Decrypted);
			byte[] aEncrypted = Encrypt(aDecrypted, seedKey, cbcPad);
			...
			if (cbcPad)
			{
				...
				if (nPadding > 0 && nPadding <= 16)
				{
					...
					if (i == nOutSize - nPadding)
					{
						...
						nOutSize -= nPadding;
						byte[] TempData = new byte[nOutSize];
						Buffer.BlockCopy(OutData, 0, TempData, 0, nOutSize);
						OutData = new byte[nOutSize];
						Buffer.BlockCopy(TempData, 0, OutData, 0, nOutSize);
					}
				}
			}
			return OutData;
		...
		{
			return CSeed.Decrypt(Data, seedKey, false);
		}
		public static string Decrypt(string Encrypted, byte[] seedKey, bool cbcPad, Encoding Enc)
		{
			byte[] aEncrypted = Convert.FromBase64String(Encrypted);
			byte[] aDecrypted = Decrypt(aEncrypted, seedKey, cbcPad);
			return Enc.GetString(aDecrypted);
		}
